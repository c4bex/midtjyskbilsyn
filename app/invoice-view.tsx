...
export function InvoiceView({ onNotify }: { onNotify: (message: string) => void }) {
  ...
  const save = async (changes: Partial<Invoice>) => { update(changes); const next = { ...selected, ...changes }; await fetch("/api/invoices", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ id: next.id, description: next.description, quantity: next.quantity, unitPriceOre: Math.round(next.price * 100), status: next.status }) }); onNotify("Fakturaændringen er gemt"); };
  return <div className="module-view invoice-view">
    ...
    <button className="secondary-button" onClick={() => void save({ status: selected.status })}>Gem ændringer</button>
  </div>;
}
