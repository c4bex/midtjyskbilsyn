...
export function InvoiceView({ onNotify }: { onNotify: (message: string) => void }) {
  ...
  useEffect(() => { fetch("/api/invoices", { cache: "no-store" }).then((response) => response.ok ? response.json() as Promise<{ invoices: Array<{ id: string; customer_name: string; period: string; description: string; quantity: number; unit_price_ore: number; status: Invoice["status"] }> }> : Promise.reject()).then((data) => setInvoices(data.invoices.map((invoice) => ({ id: invoice.id, customer: invoice.customer_name, period: invoice.period, description: invoice.description, quantity: invoice.quantity, price: invoice.unit_price_ore / 100, status: invoice.status, registration: "" })))).catch(() => undefined).finally(() => setLoading(false)); }, []);
  const save = async (changes: Partial<Invoice>) => { if (!selected) return; update(changes); const next = { ...selected, ...changes }; await fetch("/api/invoices", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ id: next.id, description: next.description, quantity: next.quantity, unitPriceOre: Math.round(next.price * 100), status: next.status }) }); onNotify("Fakturaændringen er gemt"); };
