...
type PublicVehicleLookup = { found: boolean; status: "connected" | "unavailable" | "not_checked"; vehicle?: { make?: string | null; model?: string | null; inspectionDate?: string | null; nextInspectionDate?: string | null } };
...
export default function PublicBookingPage() {
  ...
  const lookupVehicle = async (plate: string) => {
    ...
    try {
      const response = await fetch(`/api/public/vehicle-lookup?plate=${encodeURIComponent(plate)}`, { cache: "no-store" });
      const data = await response.json() as PublicVehicleLookup;
      setVehicleLookup(response.ok ? data : { found: false, status: "unavailable" });
    } catch {
      setVehicleLookup({ found: false, status: "unavailable" });
