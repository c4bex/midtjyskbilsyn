...
export default function BranchekundePage() {
  ...
  const submitForgot = async (event: FormEvent) => { event.preventDefault(); setLoginError(""); const response = await fetch("/api/portal/forgot-password", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email: login.email }) }); if (response.ok) setForgotSent(true); else setLoginError("Kunne ikke sende forespørgslen lige nu. Prøv igen om et øjeblik."); };
  ...
  if (!session) return <main className="business-portal"><form className="business-login" onSubmit={forgotMode ? submitForgot : submitLogin}><span className="business-logo"><Building2 size={20} /></span><p className="public-eyebrow">BRANCHEKUNDEPORTAL</p><h1>{forgotMode ? "Glemt adgangskode" : "Log ind"}</h1><p>{forgotMode ? "Indtast din e-mail, så sender vi et sikkert nulstillingslink." : "Book og administrér tider for din virksomhed."}</p><label>E-mail<input type="email" required value={login.email} onChange={(event) => setLogin({ ...login, email: event.target.value })} /></label>{!forgotMode && <label>Adgangskode<input type="password" required value={login.password} onChange={(event) => setLogin({ ...login, password: event.target.value })} /></label>}{loginError && <div className="business-error">{loginError}</div>}{forgotMode && forgotSent && <div className="business-success">Hvis kontoen findes, er der sendt et nulstillingslink.</div>}{!forgotSent && <button className="business-primary">{forgotMode ? "Send nulstillingslink" : "Log ind"} <ChevronRight size={17} /></button>}{!forgotMode && <button type="button" className="business-link-button" onClick={() => { setForgotMode(true); setLoginError(""); }}>Glemt adgangskode?</button>}{forgotMode && <button type="button" className="business-link-button" onClick={() => { setForgotMode(false); setForgotSent(false); setLoginError(""); }}>Tilbage til login</button>}</form></main>;
