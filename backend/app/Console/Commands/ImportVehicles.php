...
class ImportVehicles extends Command
{
    ...
    public function handle(): int
    {
        ...
        if (! is_array($rows)) {
            $this->error('Ugyldig JSON eller manglende vehicles-array.');
        ...
        foreach ($rows as $index => $row) {
            ...
            if ($plate === '' || $name === '') {
                $this->warn('Springer række '.($index + 1).' over.');
        ...
        if (! $this->option('write')) {
            $this->comment('Forhåndsvisning: ingen data er skrevet. Brug --write efter godkendelse.');

            return self::SUCCESS;
        }
        if (! $this->confirm('Skriv disse data til den lokale MySQL-database?', false)) {
            return self::SUCCESS;
        }
        ...
        DB::transaction(function () use ($valid, $batchId): void {
            foreach ($valid as $item) {
                ...
                if (! $customerId) {
                    $customerId = DB::table('customers')->insertGetId(['display_name' => $item['name'], 'customer_type' => $item['type'], 'created_at' => now(), 'updated_at' => now()]);
                }
